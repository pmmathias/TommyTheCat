...
var ProcessAudioBufferSource = function () {
  ...
  _createClass(ProcessAudioBufferSource, [{
    ...
    value: function extract(target) {
      ...
      var position = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
      this.position = position;
      var len = this.leftChannel.length;
      var i = 0;
      for (; i < numFrames; i++) {
        // Wrap-around: the buffer becomes a seamless infinite source so
        // SoundTouch never hits end-of-stream and its FFT windows stay
        // continuous across the loop boundary.
        var idx = (i + position) % len;
...
var SoundTouchWorklet = function (_AudioWorkletProcesso) {
  ...
  _createClass(SoundTouchWorklet, [{
    ...
    value: function process(inputs, outputs) {
      ...
      if (!framesExtracted) {
        // In-place seamless loop: rewind source+filter, re-extract.
        // Avoids main-thread round-trip that created an audible gap.
        try {
          this._filter.sourcePosition = 0;
          this._filter._position = 0;
          this._filter.outputBufferPosition = 0;
          if (this._filter.clear) this._filter.clear();
          if (this.bufferSource) this.bufferSource._position = 0;
        } catch (e) {}
        framesExtracted = this._filter.extract(samples, inputs[0][0].length);
        if (!framesExtracted) {
